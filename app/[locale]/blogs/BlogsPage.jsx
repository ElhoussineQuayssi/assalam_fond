...
export default function BlogsPage() {
  ...
  const categories = useMemo(() => {
    const uniqueCategories = [...new Set(blogs.map(post => post.category).filter(Boolean))];
  ...
  useEffect(() => {
    ...
    // Filter by category
    if (selectedCategory && selectedCategory !== categories[0]) { // categories[0] is "All"
    ...
    <main className="bg-slate-50/50 min-h-screen">
      ...
      <div className="container mx-auto px-6 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              ...
              {filteredPosts.slice(0, visiblePosts).map((post, index) => (
                ...
                >
                  ...
                  <div className="p-8">
                    ...
                    <MarqueeText text={post.title} className="text-xl font-bold mb-3 group-hover:text-blue-600 transition-colors" />
                    <p className="text-slate-500 text-sm line-clamp-2 mb-6">
                      {post.excerpt}
                    </p>
                    <Link href={`/blogs/${post.id}`} className="flex items-center gap-2 text-blue-600 font-black text-xs group-hover:gap-4 transition-all">
          ...
          {/* Sticky Sidebar */}
          <aside className="lg:col-span-4 space-y-8 h-fit sticky top-28" id="blog-sidebar">
            {/* Search Box */}
            <div className="sidebar-widget p-8 bg-slate-50 backdrop-blur-md rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-50">
              <h4 className="font-bold mb-4 text-sm">{t('sidebar.categories_title')}</h4>
            ...
            {/* Recent Posts */}
            <div className="sidebar-widget p-8 bg-white backdrop-blur-md rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-50">
              <h4 className="font-bold mb-4 text-sm">{t('sidebar.recent_posts_title')}</h4>
