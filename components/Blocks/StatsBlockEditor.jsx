...
const StatsBlockEditor = ({ content = {}, onChange, isDarkMode = false }) => {
  ...
  const isStatValid = (stat) => {
    return stat.label && stat.label.trim() !== "" && 
    ...
    <div className="space-y-4">
      <Card className={`${isDarkMode ? 'bg-slate-700 border-slate-600' : 'bg-gray-50 border-gray-200'}`}>
        ...
        <CardContent className="space-y-4">
            ...
            {safeStats.length === 0 ? (
              ...
              <div className="space-y-3">
                {safeStats.map((stat, statIndex) => (
                  <Card key={statIndex} className={`${isDarkMode ? 'bg-slate-800 border-slate-600' : 'bg-white border-gray-200'}`}>
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                          ...
                          <div className="flex gap-2">
                            <Input
                              value={stat.value || ""}
                              onChange={(e) => handleValueChange(statIndex, e.target.value)}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>
                              {stat.value ? `Value: ${stat.value}` : "No value set"}
                            </p>
                          </div>
                        </div>
                      </div>
          ...
          {/* Example Stats */}
          {(safeStats.length === 0) && (
            <div className={`p-4 rounded-lg border ${isDarkMode ? 'bg-slate-700 border-slate-600' : 'bg-blue-50 border-blue-200'}`}>
          ...
          <div className="mt-4 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
            <p className={`text-xs ${isDarkMode ? 'text-blue-300' : 'text-blue-700'}`}>
              <strong>Validation:</strong> Each statistic should have both a label and a numeric value for best display.
            </p>
          </div>
        </CardContent>
      </Card>
