    ...
    if (!loading && projects.length > 0) {
      ...
      cards.forEach(card => {
        ...
        const handleMouseEnter = () => {
          ...
          gsap.to(card, { boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.25)", duration: 0.3, ease: "power2.out" });
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            ...
            projects.map((project, index) => {
              ...
              return (
                <Link key={project.id} href={`/projects/${project.id}`} className="project-card group bg-white border border-slate-100 rounded-[2rem] p-6 hover:shadow-2xl transition-all duration-500">
                  {project.image && (
