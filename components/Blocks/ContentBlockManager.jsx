...
import { Plus, Edit, Trash2, ChevronUp, ChevronDown, Eye, Type, BarChart3, Settings, GraduationCap, TrendingUp, Clock, ImageIcon, CheckCircle2 } from "lucide-react";
...
const ContentBlockManager = ({ 
  ...
  const blockTypes = [
    { value: "text", label: "Text Block", icon: Type, description: "Simple text with heading" },
    { value: "services", label: "Services Block", icon: Settings, description: "Organized services and categories" },
    { value: "stats", label: "Stats Block", icon: BarChart3, description: "Statistics and key metrics" },
    { value: "programme", label: "Programme Block", icon: GraduationCap, description: "Programme with modules" },
    { value: "impact", label: "Impact Block", icon: TrendingUp, description: "Impact and results" },
    { value: "sponsorship", label: "Sponsorship Block", icon: TrendingUp, description: "Sponsorship impact and formulas" },
    { value: "timeline", label: "Timeline Block", icon: Clock, description: "Project timeline events" },

    { value: "list", label: "List Block", icon: CheckCircle2, description: "Organized lists with titles and descriptions" }
  ];
  ...
  const handleDeleteBlock = (blockId) => {
    if (confirm("Are you sure you want to delete this block?")) {
      const updatedBlocks = safeContentBlocks.filter(block => block.id !== blockId);
      onChange(updatedBlocks);
  ...
  const handleMoveBlock = (blockId, direction) => {
    const currentIndex = safeContentBlocks.findIndex(block => block.id === blockId);
    if (
  ...
  const renderBlockPreview = (block) => {
    ...
    switch (type) {
      case "text":
        return (
          <div className="space-y-2">
            <h4 className="font-semibold text-sm">{content.heading || "Untitled"}</h4>
      ...
      case "services":
        return (
          <div className="space-y-2">
            <h4 className="font-semibold text-sm">{content.heading || "Services"}</h4>
      ...
      case "stats":
        return (
          <div className="space-y-2">
            <h4 className="font-semibold text-sm">{content.heading || "Statistics"}</h4>
            <p className="text-sm text-gray-600 dark:text-slate-400">
      ...
      case "programme":
        return (
          <div className="space-y-2">
            <h4 className="font-semibold text-sm">{content.heading || "Programme"}</h4>
            <p className="text-sm text-gray-600 dark:text-slate-400">
              {content.duration && `${content.duration} • `}{content.modules?.length || 0} modules
            </p>
          </div>
        );
      case "impact":
        return (
          <div className="space-y-2">
            <h4 className="font-semibold text-sm">{content.heading || "Impact & Results"}</h4>
            <p className="text-sm text-gray-600 dark:text-slate-400">
      ...
      case "sponsorship":
        return (
          <div className="space-y-2">
            <h4 className="font-semibold text-sm">{content.heading || "Sponsorship Impact"}</h4>
            <p className="text-sm text-gray-600 dark:text-slate-400">
      ...
      case "timeline":
        return (
          <div className="space-y-2">
            <h4 className="font-semibold text-sm">{content.heading || "Project Timeline"}</h4>
            <p className="text-sm text-gray-600 dark:text-slate-400">
      ...
      case "list":
        return (
          <div className="space-y-2">
            <h4 className="font-semibold text-sm">{content.heading || "List Title"}</h4>
      ...
      {contentBlocks.length === 0 ? (
        ...
        <div className="space-y-4">
          {safeContentBlocks.map((block, index) => (
            <Card key={block.id} className={`${isDarkMode ? 'bg-slate-800 border-slate-600' : 'bg-white border-gray-200'}`}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                      block.type === 'text' ? 'bg-blue-100 text-blue-700' :
                      block.type === 'services' ? 'bg-green-100 text-green-700' :
                      'bg-purple-100 text-purple-700'
                    }`}>
                      {index + 1}
                    </div>
                    <div>
                      <CardTitle className={`text-base ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
                        {blockTypes.find(t => t.value === block.type)?.label || 'Unknown'}
                      </CardTitle>
                      <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>
                        {blockTypes.find(t => t.value === block.type)?.description}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setActiveEditor(activeEditor === block.id ? null : block.id)}
                      className={isDarkMode ? 'text-slate-400 hover:text-white' : ''}
                    >
