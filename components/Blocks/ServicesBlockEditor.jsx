...
const ServicesBlockEditor = ({ content = {}, onChange, isDarkMode = false }) => {
  // Ensure all arrays are properly handled
  const safeCategories = Array.isArray(content.categories) ? content.categories : [];
  ...
  const handleAddCategory = () => {
    ...
    setExpandedCategories(prev => new Set([...prev, updatedCategories.length - 1]));
  };

  const handleDeleteCategory = (categoryIndex) => {
    const updatedCategories = safeCategories.filter((_, index) => index !== categoryIndex);
    onChange({
      ...content,
      categories: updatedCategories
    });
  ...
  const handleDeleteService = (categoryIndex, serviceIndex) => {
    const updatedCategories = safeCategories.map((category, index) => {
      if (index === categoryIndex) {
        const categoryServices = Array.isArray(category.services) ? category.services : [];
        const newServices = categoryServices.filter((_, sIndex) => sIndex !== serviceIndex);
        return {
          ...category,
  ...
  return (
    <div className="space-y-4">
      <Card className={`${isDarkMode ? 'bg-slate-700 border-slate-600' : 'bg-gray-50 border-gray-200'}`}>
        <CardHeader className="pb-3">
          <CardTitle className={`text-sm ${isDarkMode ? 'text-white' : 'text-gray-800'} flex items-center gap-2`}>
        ...
        <CardContent className="space-y-4">
            ...
            {safeCategories.length === 0 ? (
              ...
              <div className="space-y-3">
                {safeCategories.map((category, categoryIndex) => (
                  <Card key={categoryIndex} className={`${isDarkMode ? 'bg-slate-800 border-slate-600' : 'bg-white border-gray-200'}`}>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            className={isDarkMode ? 'text-slate-400 hover:text-white' : 'text-gray-500'}
                          >
                            {expandedCategories.has(categoryIndex) ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                          </Button>
                          <Input
                            value={category.name || ""}
                            ...
                            className={`${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-gray-50 border-gray-300'} ${
                              !isCategoryValid(category) ? 'border-yellow-300' : ''
                            }`}
                          />
                        </div>
                        <Button
                          ...
                          {category.services?.map((service, serviceIndex) => (
                            <div key={serviceIndex} className={`p-3 rounded-lg border ${
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                <Input
                                  value={service.name || ""}
                                  onChange={(e) => handleServiceChange(categoryIndex, serviceIndex, 'name', e.target.value)}
                              ...
                              <Textarea
                                ...
                                onChange={(e) => handleServiceChange(categoryIndex, serviceIndex, 'description', e.target.value)}
          ...
          <div className="mt-4 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
            <p className={`text-xs ${isDarkMode ? 'text-blue-300' : 'text-blue-700'}`}>
              <strong>Validation:</strong> Each category should have a name and at least one service for best results.
            </p>
          </div>
        </CardContent>
      </Card>
