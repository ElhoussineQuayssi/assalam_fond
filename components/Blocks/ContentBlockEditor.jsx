...
import { FileText, Settings, BarChart3, Eye, Type, GraduationCap, TrendingUp, Clock, ImageIcon, CheckCircle2 } from "lucide-react";
          ...
          content.map((block) => {
            ...
            switch (type) {
                        ...
                        <div key={index} className="space-y-4">
                          ...
                          <div className="grid gap-3">
                            {category.services?.map((service, serviceIndex) => (
                              <div key={serviceIndex} className="p-4 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg">
                                {service.name && (
                  ...
                  <div key={id} className="py-10 max-w-5xl mx-auto px-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {blockContent.stats?.map((stat, index) => (
                        <div key={index} className="p-6 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg text-center">
                  ...
                  <div key={id} className="py-10 max-w-5xl mx-auto px-6">
                    <div className="flex flex-col md:flex-row gap-6">
                      <div className="md:w-1/4 bg-blue-600 text-white p-6 rounded-3xl shadow-lg shadow-blue-100">
                        <h3 className="text-md font-bold mb-4">{blockContent.heading}</h3>
                        <div className="flex items-center gap-2 text-xs">{blockContent.duration}</div>
                      </div>
                      <div className="md:w-3/4 grid gap-3">
                        {blockContent.modules?.map((m, i) => (
                          <div key={i} className="p-4 bg-white border border-slate-100 rounded-2xl hover:border-blue-200 transition-colors">
                            <h4 className="text-xs font-bold text-blue-600 uppercase mb-1">{m.title}</h4>
              ...
              case "sponsorship":
                return (
                  <div key={id} className="py-10 max-w-5xl mx-auto px-6">
                    <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-8 text-center">{blockContent.heading}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {(blockContent.impacts || blockContent.formulas)?.map((item, i) => (
                        <div key={i} className="p-6 bg-white border border-slate-100 rounded-[2rem] text-center hover:shadow-md transition-shadow">
                          <div className="w-8 h-8 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-3">
                            <TrendingUp size={14} className="text-blue-500" />
                          </div>
                          <h4 className="text-xs font-bold text-slate-800 mb-2">{item.description || item.name}</h4>
                          <p className="text-[11px] text-slate-500 leading-relaxed uppercase">{item.value || item.amount}</p>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              case "timeline":
                return (
                  <div key={id} className="py-10 max-w-3xl mx-auto px-6">
                    <h3 className="text-sm font-black mb-6 text-blue-600 uppercase">{blockContent.heading}</h3>
                    <div className="space-y-6 border-l-2 border-blue-50 ml-2">
                      {blockContent.events?.map((ev, i) => (
                        <div key={i} className="relative pl-8">
                          ...
                          <span className="text-[10px] font-bold text-slate-400 uppercase">{ev.year}</span>
                  ...
                  <div key={id} className="py-10 max-w-5xl mx-auto px-6">
                    ...
                    <div className="space-y-4">
                      {(blockContent.items || []).map((item, index) => (
                        <div key={index} className="flex items-start gap-4 p-4 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg">
              ...
              default:
                return (
                  <div key={id} className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded">
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
            <div className={`p-3 rounded-lg text-center ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
              <div className="text-lg font-bold text-blue-600">{blockStats.total}</div>
              <div className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>Total Blocks</div>
            </div>
            <div className={`p-3 rounded-lg text-center ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
              <div className="text-lg font-bold text-blue-600">{blockStats.text}</div>
              <div className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>Text</div>
            </div>
            <div className={`p-3 rounded-lg text-center ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
              <div className="text-lg font-bold text-green-600">{blockStats.services}</div>
              <div className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>Services</div>
            </div>
            <div className={`p-3 rounded-lg text-center ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
              <div className="text-lg font-bold text-purple-600">{blockStats.stats}</div>
              <div className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>Stats</div>
            </div>
            <div className={`p-3 rounded-lg text-center ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
              <div className="text-lg font-bold text-orange-600">{blockStats.programme}</div>
              <div className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>Programme</div>
            </div>
            <div className={`p-3 rounded-lg text-center ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
              <div className="text-lg font-bold text-red-600">{blockStats.timeline}</div>
              <div className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>Timeline</div>
            </div>
          </div>
      ...
      {showPreview ? (
        <Card className={`${isDarkMode ? 'bg-slate-800 border-slate-600' : 'bg-white border-gray-200'}`}>
          ...
          <CardContent>
            <div className={`min-h-[400px] p-6 rounded-lg border-2 border-dashed ${
              isDarkMode ? 'border-slate-600 bg-slate-900/20' : 'border-gray-300 bg-gray-50'
            }`}>
      ...
      {/* Help Section */}
      <Card className={`${isDarkMode ? 'bg-slate-800 border-slate-600' : 'bg-blue-50 border-blue-200'}`}>
        <CardContent className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-xs text-gray-600 dark:text-slate-400">
            <div>
              <strong className="text-blue-600 dark:text-blue-400">Text Block:</strong>
              <p>Simple text content with heading and description for general information sections.</p>
            </div>
            <div>
              <strong className="text-green-600 dark:text-green-400">Services Block:</strong>
              <p>Organized services grouped by categories, perfect for showcasing what you offer.</p>
            </div>
            <div>
              <strong className="text-purple-600 dark:text-purple-400">Stats Block:</strong>
              <p>Key statistics and metrics with labels and values for impact measurement.</p>
            </div>
            <div>
              <strong className="text-orange-600 dark:text-orange-400">Programme Block:</strong>
              <p>Educational programmes with modules and duration information.</p>
            </div>
            <div>
              <strong className="text-red-600 dark:text-red-400">Impact Block:</strong>
              <p>Impact metrics and results with descriptions and values.</p>
            </div>
            <div>
              <strong className="text-yellow-600 dark:text-yellow-400">Sponsorship Block:</strong>
              <p>Sponsorship impact with formulas and monetary amounts.</p>
            </div>
            <div>
              <strong className="text-indigo-600 dark:text-indigo-400">Timeline Block:</strong>
              <p>Project timeline with events, years, and descriptions.</p>
            </div>

            <div>
              <strong className="text-cyan-600 dark:text-cyan-400">List Block:</strong>
