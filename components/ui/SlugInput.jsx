...
import { Copy, RefreshCw, Eye, CheckCircle, AlertCircle, Edit3, Save } from "lucide-react";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { generateSlug, validateSlug, copySlugToClipboard } from "@/utils/slugGenerator";
...
export default function SlugInput({
  ...
  const getValidationColor = () => {
    if (!internalValue) return isDarkMode ? 'border-slate-600' : 'border-gray-300';
    if (validation.isValid) return isDarkMode ? 'border-green-500' : 'border-green-500';
    ...
    <div className={`space-y-2 ${className}`}>
      ...
      {showSuggestions && title && (
        <div className={`p-3 rounded border ${
          ...
          <p className={`text-xs font-medium mb-2 ${isDarkMode ? 'text-slate-300' : 'text-gray-700'}`}>
            URL Preview
          </p>
          <div className="space-y-1">
            <div className={`text-xs font-mono ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>
              https://yoursite.com/projects/{internalValue || 'your-project-slug'}
            </div>
            {internalValue && (
