...
export default function GoalsManager({
  ...
  const handleAddGoal = () => {
    if (newGoal.trim() && goals.length < maxGoals) {
      const updatedGoals = [...goals, { id: Date.now(), text: newGoal.trim(), priority: goals.length + 1 }];
    ...
    <div className={`space-y-4 ${className}`}>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Badge variant="outline" className={`text-xs ${isDarkMode ? 'border-slate-600 text-slate-400' : 'border-gray-300 text-gray-600'}`}>
                Priority Order
              </Badge>
            </div>
          </div>
                ...
                <div className={`flex flex-col items-center gap-1 ${
                  ...
                  <Badge 
                    ...
                    className={`text-xs px-1.5 py-0.5 ${
                      isDarkMode ? 'border-slate-500 text-slate-400' : 'border-gray-300 text-gray-500'
                    }`}
                  >
      ...
      {canAdd && (
        <div className={`p-4 border-2 border-dashed rounded-lg ${
          ...
          <p className={`text-xs mt-2 ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>
            Press Enter or click "Add Goal" to add. Minimum {minGoals}, maximum {maxGoals} goals.
          </p>
        </div>
      )}
      ...
      {!canAdd && goals.length >= maxGoals && (
        <div className={`p-3 rounded-lg ${
          ...
          <p className="text-xs mt-1">
            You've reached the maximum of {maxGoals} goals. Remove some goals to add new ones.
          </p>
        </div>
      )}
      ...
      {goals.length < minGoals && (
        <div className={`p-3 rounded-lg ${
          ...
          <p className="text-xs mt-1">
            You need at least {minGoals} goal{plural(minGoals)} for this project.
          </p>
        </div>
      )}
      ...
      {goals.length > 0 && (
        <div className={`flex items-center justify-between text-xs ${
          ...
          <span>{goals.length} goal{plural(goals.length)} defined</span>
          <span>
            Priority: {goals[0]?.priority || 1} - {goals[goals.length - 1]?.priority || goals.length}
          </span>
        </div>
      )}
    </div>
