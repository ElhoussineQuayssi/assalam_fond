...
export default function Navbar() {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      gsap.fromTo(".mobile-link", 
        { opacity: 0, x: 20 }, 
        { opacity: 1, x: 0, stagger: 0.1, duration: 0.5, ease: "power2.out", delay: 0.3 }
      );
    } else {
  ...
  const handleMouseEnter = () => {
    const handleMouseMove = (e) => {
      ...
      if (distance < maxDistance) {
        ...
        gsap.to(contactRef.current, { x: moveX, y: moveY, duration: 0.3, ease: "power2.out" });
      } else {
        gsap.to(contactRef.current, { x: 0, y: 0, duration: 0.3, ease: "power2.out" });
      }
    };
  ...
  const handleMouseLeave = () => {
    if (contactRef.current._mouseMoveHandler) {
      document.removeEventListener('mousemove', contactRef.current._mouseMoveHandler);
    }
    gsap.to(contactRef.current, { x: 0, y: 0, duration: 0.3, ease: "power2.out" });
  };
    ...
    <nav className={`fixed top-0 w-full z-[100] transition-all duration-300
      ...
      <div className="container mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className={`hidden md:flex items-center gap-10 font-medium transition-colors ${
          ...
          <a href="/contact" ref={contactRef} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className="bg-blue-600 text-white px-8 py-2.5 rounded-full text-sm font-bold shadow-lg shadow-blue-200 active:scale-95 transition-all">
            {t('contact')}
          </a>
        </div>
        ...
        <button className="md:hidden" onClick={() => setIsOpen(true)}>
          <Menu size={28} className={isScrolled ? "text-blue-600" : "text-white"} />
        </button>
      </div>

      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 z-[110] bg-white transition-transform duration-500 ease-in-out
        ...
        <div className="flex flex-col gap-6 p-10">
          {[
            ...
            <a key={i} href={link.href} onClick={() => setIsOpen(false)} className="mobile-link text-2xl font-bold text-slate-900 border-b border-slate-50 pb-4">
              {link.text}
            </a>
          ))}
        </div>
