...
export default function BlogForm({ 
      ...
      if (blog && blog.id) {
        ...
        try {
          ...
          setLocalFormData({
            ...
            tags: blog.tags || '',
            image: blog.image || '',
            title: { fr: blog.title || '', en: blog.title_en || '', ar: blog.title_ar || '' },
            excerpt: { fr: blog.excerpt || '', en: blog.excerpt_en || '', ar: blog.excerpt_ar || '' },
            content: { fr: blog.content || '', en: blog.content_en || '', ar: blog.content_ar || '' }
          });
        } finally {
          setIsLoading(false);
        }
  ...
  const completedLanguages = ['fr', 'en', 'ar'].filter(lang => {
    ...
    return title && title.trim() !== '' && excerpt && excerpt.trim() !== '' && content && content.trim() !== '';
  });
  ...
  const handleTitleChange = (language, title) => {
    setLocalFormData(prev => ({
      ...
      slug: language === 'fr' && (!prev.slug || prev.slug === title.toLowerCase().replace(/\s+/g, '-'))
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {(language, context) => (
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <label htmlFor={`blog-title-${language}`} className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                        {language === 'fr' ? 'Titre' : language === 'en' ? 'Title' : 'العنوان'} ({context.languageCode.toUpperCase()}) *
                      </label>
                      <Input
                    ...
                    <div>
                      <label htmlFor={`blog-excerpt-${language}`} className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                        {language === 'fr' ? 'Extrait' : language === 'en' ? 'Excerpt' : 'ملخص'}
                      </label>
                      <textarea
                        ...
                        placeholder={language === 'fr' ? 'Brève description en français...' : language === 'en' ? 'Brief description in English...' : 'وصف مختصر بالعربية...'}
                        dir={context.isRTL ? 'rtl' : 'ltr'}
                      />
                    ...
                    <div>
                      <label htmlFor={`blog-content-${language}`} className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                        {language === 'fr' ? 'Contenu' : language === 'en' ? 'Content' : 'المحتوى'}
                      </label>
                      <textarea
                        id={`blog-content-${language}`}
                        className={`w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-700 dark:border-slate-600 dark:text-white ${
                          localValidationErrors.content && localValidationErrors.content[language] ? 'border-red-500' : ''
                        }`}
                        rows={12}
                        value={localFormData.content[language] || ''}
                        onChange={(e) => handleMultilingualChange('content', language, e.target.value)}
                        placeholder={language === 'fr' ? 'Écrivez le contenu de votre article de blog en français...' : language === 'en' ? 'Write your blog post content in English...' : 'اكتب محتوى مقالتك بالعربية...'}
                        dir={context.isRTL ? 'rtl' : 'ltr'}
                      />
                      {localValidationErrors.content && localValidationErrors.content[language] && (
                        <p className="text-red-500 text-xs mt-1">{localValidationErrors.content[language]}</p>
                      )}
                    </div>
            ...
            <div className="border-t border-gray-200 dark:border-gray-600 pt-6">
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">
                {activeLanguage === 'fr' ? 'Champs Partagés' : activeLanguage === 'en' ? 'Shared Fields' : 'الحقول المشتركة'}
              </h3>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="blog-slug" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                    {activeLanguage === 'fr' ? 'Slug' : activeLanguage === 'en' ? 'Slug' : 'الرابط'}
                  </label>
                  <Input
                    id="blog-slug"
                    value={localFormData.slug}
                    onChange={(e) => handleInputChange('slug', e.target.value)}
                    placeholder={activeLanguage === 'fr' ? 'auto-généré-à-partir-du-titre' : activeLanguage === 'en' ? 'auto-generated-from-title' : 'يتم-إنشاءه-تلقائيًا-من-العنوان'}
                    className="dark:bg-slate-700 dark:border-slate-600 dark:text-white"
                  />
                </div>
                
                <div>
                  <label htmlFor="blog-category" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                    {activeLanguage === 'fr' ? 'Catégorie' : activeLanguage === 'en' ? 'Category' : 'الفئة'}
                  </label>
                  <Select value={localFormData.category} onValueChange={(value) => handleInputChange('category', value)}>
                    <SelectTrigger className="dark:bg-slate-700 dark:border-slate-600 dark:text-white">
                      <SelectValue placeholder={activeLanguage === 'fr' ? 'Sélectionner une catégorie' : activeLanguage === 'en' ? 'Select category' : 'اختر الفئة'} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="benevolat">{activeLanguage === 'fr' ? 'Benevolat' : activeLanguage === 'en' ? 'Benevolat' : 'التطوع'}</SelectItem>
                      <SelectItem value="education">{activeLanguage === 'fr' ? 'Education' : activeLanguage === 'en' ? 'Education' : 'التعليم'}</SelectItem>
                      <SelectItem value="social">{activeLanguage === 'fr' ? 'Social' : activeLanguage === 'en' ? 'Social' : 'الاجتماعي'}</SelectItem>
                      <SelectItem value="economic">{activeLanguage === 'fr' ? 'Economic' : activeLanguage === 'en' ? 'Economic' : 'الاقتصادي'}</SelectItem>
                    </SelectContent>
                  </Select>
                  {localValidationErrors.category && (
                    <p className="text-red-500 text-xs mt-1">{localValidationErrors.category}</p>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                  <label htmlFor="blog-status" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                    {activeLanguage === 'fr' ? 'Statut' : activeLanguage === 'en' ? 'Status' : 'الحالة'}
                  </label>
                  <Select value={localFormData.status} onValueChange={handleStatusChange}>
                    <SelectTrigger className="dark:bg-slate-700 dark:border-slate-600 dark:text-white">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="draft">{activeLanguage === 'fr' ? 'Brouillon' : activeLanguage === 'en' ? 'Draft' : 'مسودة'}</SelectItem>
                      <SelectItem value="published">{activeLanguage === 'fr' ? 'Publié' : activeLanguage === 'en' ? 'Published' : 'منشور'}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                
                <div>
                  <label htmlFor="blog-published-at" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                    {activeLanguage === 'fr' ? 'Date de publication' : activeLanguage === 'en' ? 'Published At' : 'تاريخ النشر'}
                  </label>
                  <Input
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                  <label htmlFor="blog-tags" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                    {activeLanguage === 'fr' ? 'Mots-clés' : activeLanguage === 'en' ? 'Tags' : 'الوسوم'}
                  </label>
                  <Input
                    id="blog-tags"
                    value={localFormData.tags}
                    onChange={(e) => handleInputChange('tags', e.target.value)}
                    placeholder={activeLanguage === 'fr' ? 'mot-clé1, mot-clé2, mot-clé3' : activeLanguage === 'en' ? 'tag1, tag2, tag3' : 'وسم1، وسم2، وسم3'}
                    className="dark:bg-slate-700 dark:border-slate-600 dark:text-white"
                  />
                </div>
                
                <div>
                  <label htmlFor="blog-image" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                    {activeLanguage === 'fr' ? 'URL de l\'image' : activeLanguage === 'en' ? 'Image URL' : 'رابط الصورة'}
                  </label>
                  <Input
                    id="blog-image"
                    value={localFormData.image}
                    onChange={(e) => handleInputChange('image', e.target.value)}
                    placeholder={activeLanguage === 'fr' ? 'https://example.com/image.jpg' : activeLanguage === 'en' ? 'https://example.com/image.jpg' : 'https://example.com/image.jpg'}
                    className="dark:bg-slate-700 dark:border-slate-600 dark:text-white"
                  />
                </div>
              </div>
