      ...
      for (const comment of sampleComments) {
        try {
          ...
          if (response.ok) {
            ...
            errors.push(`Failed to add comment from ${comment.author_name}: ${error.error}`);
          }
        } catch (error) {
          errors.push(`Error adding comment from ${comment.author_name}: ${error.message}`);
        }
      }
      ...
      for (const message of sampleMessages) {
        try {
          ...
          if (response.ok) {
            ...
            const error = await response.json();
            errors.push(`Failed to add message from ${message.name}: ${error.error}`);
          }
        } catch (error) {
          errors.push(`Error adding message from ${message.name}: ${error.message}`);
        }
      }
    ...
    <Card className="mb-6">
      ...
      <CardContent>
        <p className="text-sm text-gray-600 mb-4">
          Add sample comments and messages to test the admin panel functionality.
        </p>
        <Button 
