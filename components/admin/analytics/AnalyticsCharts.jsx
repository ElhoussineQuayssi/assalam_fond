    ...
    const fetchChartData = async () => {
      try {
        ...
        const response = await fetch(
          `/api/analytics/charts?period=${period}&chartType=${chartType}`,
        );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        {[...Array(2)].map((_, index) => (
          ...
          >
            <CardHeader>
              <div
                className={`h-6 bg-gray-200 rounded animate-pulse ${isDarkMode ? "bg-slate-700" : ""}`}
              ></div>
            </CardHeader>
            <CardContent>
              <div className="h-64 flex items-center justify-center">
                <Loader2
                  className={`h-8 w-8 animate-spin ${isDarkMode ? "text-slate-400" : "text-gray-400"}`}
                />
              </div>
            </CardContent>
          </Card>
    ...
    <div ref={chartRef} className="space-y-6 mb-8">
      ...
      <div className="flex gap-4 items-center">
        <Select value={period} onValueChange={setPeriod}>
          <SelectTrigger
            className={`w-32 ${isDarkMode ? "bg-slate-800 border-slate-600" : "bg-white border-slate-200"}`}
          >
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
