...
export default function AnalyticsMetrics({ isDarkMode = false, period = '30d' }) {
        ...
        {[...Array(4)].map((_, index) => (
          ...
          >
            <CardHeader className="pb-2">
              <div className={`h-4 bg-gray-200 rounded animate-pulse ${isDarkMode ? 'bg-slate-700' : ''}`}></div>
            </CardHeader>
            <CardContent>
              <div className={`h-8 bg-gray-200 rounded animate-pulse mb-2 ${isDarkMode ? 'bg-slate-700' : ''}`}></div>
              <div className={`h-3 bg-gray-200 rounded animate-pulse w-2/3 ${isDarkMode ? 'bg-slate-700' : ''}`}></div>
            </CardContent>
          </Card>
        ))}
      </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      {metricsData.map((metric, index) => (
        ...
        >
          <CardHeader className="pb-2">
            <CardTitle className={`text-sm font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>
              {metric.title}
            </CardTitle>
          </CardHeader>
          <CardContent>
