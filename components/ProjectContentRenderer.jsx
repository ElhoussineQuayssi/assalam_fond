...
import { CheckCircle2, Clock, GraduationCap, Target, TrendingUp } from "lucide-react";
...
const TextBlock = ({ heading, text }) => (
  <section className="py-6 max-w-3xl mx-auto px-6">
    <h2 className="text-lg font-bold text-slate-800 mb-3 border-l-4 border-blue-500 pl-4">{heading}</h2>
  ...
  <section className="py-10 max-w-5xl mx-auto px-6">
    <div className="flex flex-col md:flex-row gap-6">
      <div className="md:w-1/4 bg-blue-600 text-white p-6 rounded-3xl shadow-lg shadow-blue-100">
        ...
        <div className="flex items-center gap-2 text-xs"><Clock size={14}/> {duration}</div>
      </div>
...
const ImpactBlock = ({ heading, impacts, formulas }) => {
  ...
  return (
  <section className="py-10 max-w-5xl mx-auto px-6">
    <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-8 text-center">{heading}</h3>
...
const TimelineBlock = ({ heading, events }) => {
  ...
  <section className="py-10 max-w-3xl mx-auto px-6">
    <h3 className="text-sm font-black mb-6 text-blue-600 uppercase">{heading}</h3>
        ...
        <div key={index} className="space-y-4">
          ...
          <div className="grid gap-3">
            {(Array.isArray(category.services) ? category.services : []).map((service, serviceIndex) => (
              <div key={serviceIndex} className="p-4 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg hover:border-blue-200 transition-colors">
                {service.name && (
  ...
  <section className="py-10 max-w-5xl mx-auto px-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {safeStats.map((stat, index) => (
        <div key={index} className="p-6 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg text-center hover:shadow-md transition-shadow">
...
const ListBlock = ({ heading, items = [] }) => {
  ...
  if (safeItems.length === 0 && (!heading || heading.trim() === '' || heading === 'List Title')) {
    return null;
  }
...
export const ProjectContentRenderer = ({ contentArray }) => {
  ...
  const filteredContent = safeContentArray.filter(block => {
    ...
    if (block.type === 'list') {
      const hasHeading = block.content?.heading && block.content.heading.trim() !== '' && block.content.heading !== 'List Title';
      const hasItems = Array.isArray(block.content?.items) && 
  ...
  return (
    <div className="space-y-6">
      {filteredContent.map((block) => {
        ...
        switch (type) {
          case 'text': return <TextBlock key={id} {...content} />;
          case 'services': return <ServicesBlock key={id} {...content} />;
          case 'stats': return <StatsBlock key={id} {...content} />;
          case 'programme': return <ProgrammeBlock key={id} {...content} />;
          case 'impact': return <ImpactBlock key={id} {...content} />;
          case 'sponsorship': return <ImpactBlock key={id} {...content} />;
          case 'timeline': return <TimelineBlock key={id} {...content} />;

          case 'list': return <ListBlock key={id} {...content} />;
          default: 
