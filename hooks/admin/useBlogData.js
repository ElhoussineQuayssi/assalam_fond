...
export function useBlogData() {
  ...
  }, [blogPage, blogPageSize, blogSearch, blogStatusFilter, blogCategoryFilter, blogSortBy, blogSortOrder]);
  ...
  const handleBlogSubmit = useCallback(async (e, formData) => {
    e.preventDefault();
    try {
      ...
      if (formDataToSubmit.title && typeof formDataToSubmit.title === 'object') {
      ...
      if (formDataToSubmit.status === 'published' && !formDataToSubmit.published_at) {
        formDataToSubmit.published_at = new Date().toISOString();
      }

      const url = currentBlog ? `/api/blog-posts/${currentBlog.id}` : '/api/blog-posts';
      ...
      toast.success(currentBlog ? 'Blog post updated successfully!' : 'Blog post created successfully!');
