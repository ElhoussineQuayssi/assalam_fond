...
const validateContentBlock = (block) => {
  const validTypes = ['text', 'services', 'stats', 'programme', 'impact', 'sponsorship', 'timeline', 'gallery', 'list'];
  if (!block || !block.type) {
    throw new Error('Content block must have a type');
  }
  
  if (!validTypes.includes(block.type)) {
    throw new Error(`Invalid content block type: ${block.type}. Valid types are: ${validTypes.join(', ')}`);
  }
  ...
  switch (block.type) {
    ...
    case 'services':
      ...
      if (!block.content?.categories || !Array.isArray(block.content.categories)) {
...
export const createProject = async (projectData) => {
  ...
  if (projectData.categories && typeof projectData.categories === 'string') {
    projectData.categories = projectData.categories.split(',').map(cat => cat.trim()).filter(cat => cat);
  }
  if (projectData.goals && typeof projectData.goals === 'string') {
    projectData.goals = projectData.goals.split(',').map(goal => goal.trim()).filter(goal => goal);
  }
...
export const updateProject = async (id, updateData) => {
  ...
  if (updateData.categories && typeof updateData.categories === 'string') {
    updateData.categories = updateData.categories.split(',').map(cat => cat.trim()).filter(cat => cat);
  }
  if (updateData.goals && typeof updateData.goals === 'string') {
    updateData.goals = updateData.goals.split(',').map(goal => goal.trim()).filter(goal => goal);
  }
